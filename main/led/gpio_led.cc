...
// 默认使用 LEDC 低速定时器产生 PWM，13 位占空比分辨率。
...
GpioLed::GpioLed(gpio_num_t gpio, int output_invert, ledc_timer_t timer_num, ledc_channel_t channel) {
    // 未连接 LED 的板型应使用 NoLed，避免创建无效 PWM 通道。
    assert(gpio != GPIO_NUM_NC);

    /* 配置 LEDC 定时器和通道，后续亮度/渐变只需更新 duty。 */
    ledc_timer_config_t ledc_timer = {};
    ledc_timer.duty_resolution = LEDC_TIMER_13_BIT;  // PWM 占空比分辨率
