...
public:
    ImageContent(const std::string& mime_type, const std::string& data) {
        ...
        // 二进制图片不能直接放入 JSON，先编码为 Base64 文本。
...
// MCP 工具允许返回基础值、JSON 对象或图片内容，统一用 variant 承载。
...
private:
    ...
    std::optional<int> min_value_;  // 整数最小值约束
    std::optional<int> max_value_;  // 整数最大值约束
...
public:
    ...
    std::string Call(const PropertyList& properties) {
        ...
        // 将不同 C++ 返回类型统一封装为 MCP content 数组。
