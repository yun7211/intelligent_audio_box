...
// BLUFI 同时服务配网 AP 和普通 STA 场景；优先根据 WifiManager 状态判断当前 Wi-Fi 模式。
...
// 安全上下文、连接状态和扫描状态在此统一归零，实际协议栈资源由 init() 按层创建。
Blufi::Blufi()
    : m_sec(nullptr),
...
void Blufi::_security_init() {
    // 每次 BLE 连接建立独立安全上下文，断开时立即销毁会话密钥。
...
void Blufi::_dh_negotiate_data_handler(uint8_t* data, int len, uint8_t** output_data,
    ...
    // 协商消息分阶段到达：长度 -> DH 参数 -> 对端公钥；最后计算共享密钥并派生 AES PSK。
    if (m_sec == nullptr) {
...
bool Blufi::start_wifi_scan() {
    ...
    // 已有扫描在进行时复用它，由 scan-done 事件决定是否向手机回包。
    ...
    // 扫描需要 STA 能力；纯 AP 状态下临时切到可扫描的模式。
...
void Blufi::_send_wifi_list() {
    // 只把手机需要的 SSID/RSSI 转成 BLUFI 记录，发送后清缓存并启动下一轮后台刷新。
...
void Blufi::_handle_event(esp_blufi_cb_event_t event, esp_blufi_cb_param_t* param) {
    // BLUFI 主事件状态机：管理 BLE 会话、接收凭据、驱动 WifiManager，并把结果反馈给手机。
    switch (event) {
        ...
        case ESP_BLUFI_EVENT_REQ_CONNECT_TO_AP: {
            // 手机已分段下发完凭据：先持久化，再退出其他 Wi-Fi 模式并异步尝试连接。
            ...
            xTaskCreate(
                [](void* ctx) {
                    ...
                    if (wifi.IsConnected()) {
                        // 成功报告包含当前 SSID/BSSID；回包后主动断开 BLE，结束一次配网会话。
        ...
        case ESP_BLUFI_EVENT_GET_WIFI_STATUS: {
            // 查询不只返回成功/失败，还要区分正在连接，供手机展示实时进度。
            auto& wifi = WifiManager::GetInstance();
            wifi_mode_t mode = GetWifiModeWithFallback(wifi);
            const int softap_conn_num = _get_softap_conn_num();
        ...
        case ESP_BLUFI_EVENT_GET_WIFI_LIST: {
            ...
            // 情况 1：扫描尚未结束，记录待响应标志，不能阻塞 BLUFI 任务等待。
            if (m_scan_in_progress) {
                m_send_list_after_scan = true;
                break;
            }
            // 情况 2：已有缓存则立即返回，同时后台刷新下一份缓存。
            if (!m_ap_records.empty()) {
                _send_wifi_list();
                break;
            }
            // 情况 3：无缓存则发起真实扫描；若启动失败，立即返回错误避免手机一直超时等待。
