...
// frame buffer 属于 esp_camera 驱动，使用完成必须归还；WebRTC 接管时先释放当前帧并反初始化。

Esp32Camera::Esp32Camera(const camera_config_t &config) {
    config_ = config;  // 保存配置供 WebRTC 通话结束后重新初始化
    ...
    if (s) {
        if (s->id.PID == GC0308_PID) {
            s->set_hmirror(s, 0); // GC0308 默认使用正常方向
        }
...
void Esp32Camera::Release() {
    if (streaming_on_) {
        ...
        esp_camera_deinit();
        streaming_on_ = false;
        ESP_LOGI(TAG, "Camera released for exclusive consumer");
    }
}

void Esp32Camera::Reacquire() {
    if (!streaming_on_) {
        esp_err_t err = esp_camera_init(&config_);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "Camera reacquire failed: 0x%x", err);
            return;
        }
        streaming_on_ = true;
        ESP_LOGI(TAG, "Camera reacquired");
    }
}
