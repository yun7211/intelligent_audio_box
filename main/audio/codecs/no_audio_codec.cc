...
// 本文件不配置外部编解码器寄存器，只管理 I2S 通道、DMA 与 PCM 格式。
...
// 全双工模式：收发通道共享 BCLK/WS，适用于收发脚位属于同一 I2S 外设的硬件。
...
// 单工双通道模式：扬声器 TX 与麦克风 RX 使用各自的时钟和数据引脚。
...
// PDM 变体：输出仍为标准 I2S，输入侧由硬件把 PDM 麦克风数据转换成 PCM。
