...
// 初始化顺序：建立 I2S 通道 -> 创建数据/控制接口 -> 创建输出与输入 codec 设备。
BoxAudioCodec::BoxAudioCodec(void* i2c_master_handle, int input_sample_rate, int output_sample_rate,
    ...
    // 立创实战派和 ESP-BOX-3 均使用双麦克风，并把 ES8311 的播放信号回接到
    // ES7210 MIC3。启用参考输入时向 AFE 提供 MIC1、MIC2、MIC3，格式为 MMR。
    ...
    // 输入链路：复用 I2S 数据接口，通过另一 I2C 地址控制 ES7210 ADC。
...
void BoxAudioCodec::EnableInput(bool enable) {
    ...
    if (enable) {
        ...
        if (input_reference_) {
            // MIC1/MIC2 是近端人声，MIC3 是 ES8311 播放回灌参考；三路按此顺序
            // 打包后，AfeAudioProcessor/AfeWakeWord 会根据 input_channels_=3 生成 MMR。
