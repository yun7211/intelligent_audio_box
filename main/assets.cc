...
#endif
    // 单例创建时立即定位并挂载资源分区，后续模块可直接按名称取资源。
    InitializePartition();
}
...
bool Assets::LoadSrmodelsFromIndex(Assets* assets, cJSON* root) {
    // 语音模型路径表来自 index.json；root 未提供时从当前资源分区读取并解析。
    void* ptr = nullptr;
    size_t size = 0;
    bool need_delete_root = false;

    // 调用方未提供索引对象时自行加载 index.json。
    if (root == nullptr) {
...
bool Assets::LvglStrategy::InitializePartition(Assets* assets) {
    // LVGL 资源使用自定义表头，整分区 mmap 后按 offset 零拷贝访问字体和图片。
...
bool Assets::EmoteStrategy::InitializePartition(Assets* assets) {
    // Emote 显示方案通过 SPIFFS 挂载资源分区，由文件系统负责名称到数据的映射。
    assets->partition_valid_ = false;
    ...
    if (emote_display && emote_display->GetEmoteHandle() != nullptr) {
        const emote_data_t data = {
            ...
            .flags = {
                .mmap_enable = true, // 模型加载依赖 mmap，必须保持开启
            },
        };
...
bool Assets::Download(std::string url, std::function<void(int progress, size_t speed)> progress_callback) {
    // 下载前暂时卸载旧资源；新镜像按扇区擦除并流式写入，完整性通过头部与长度检查。
    ...
    // 写 Flash 前解除 mmap/文件系统挂载，避免仍有读者访问正在覆盖的分区。
    UnApplyPartition();
    ...
    // 实际接收长度必须与 Content-Length 完全一致，截断镜像不能启用。
    ...
    // 负载成功写完后最后写头部，相当于提交标记；断电时不会误认半成品为有效镜像。
    if (success) {
