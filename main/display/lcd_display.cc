...
/*
 * LcdDisplay 是完整 LVGL 聊天界面实现。SetupUI() 一次性创建分层控件树；后续状态、消息、
 * 表情和预览图更新都复用这些控件。表情可来自字体、静态图片或 GIF，资源由当前主题提供。
 */
...
void LcdDisplay::InitializeLcdThemes() {
    ...
    // 内置浅色主题，资源分区可在后续 Apply() 中替换字体、背景和表情集。
...
void LcdDisplay::SetupUI() {
    // UI 树只允许创建一次，重复初始化会泄漏控件并产生多个活动屏。
    if (setup_ui_called_) {
        ESP_LOGW(TAG, "SetupUI() called multiple times, skipping duplicate call");
        return;
    }
    
    Display::SetupUI();  // 标记 UI 已初始化
