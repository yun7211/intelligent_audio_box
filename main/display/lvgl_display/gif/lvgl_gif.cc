...
// 从内存图片描述符打开 GIF，解码画布直接作为 LVGL ARGB8888 图片的数据源。
LvglGif::LvglGif(const lv_img_dsc_t* img_dsc)
    ...
    // 描述符不拥有额外像素内存，data 始终指向 gifdec 的 canvas。
...
void LvglGif::NextFrame() {
    // 定时器频繁轮询，但只有达到当前帧 delay 后才真正推进解码。
...
void LvglGif::Cleanup() {
    // 先停 LVGL 定时器再关闭解码器，防止回调继续访问已释放的画布。
