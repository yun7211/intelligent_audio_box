...
/*
 * WebRTC 媒体安全分两步：先通过 DTLS 握手验证端点并导出密钥材料，再用这些密钥建立
 * SRTP 入站/出站会话加密 RTP/RTCP。这里负责证书、mbedTLS DTLS 上下文和 libsrtp 会话
 * 的完整生命周期；实际 UDP 收发通过 cfg 中的回调接入 ICE 选中的传输通道。
 */

static int dtls_srtp_selfsign_cert(dtls_srtp_t *dtls_srtp, bool export_for_cache)
{
    // WebRTC 使用证书指纹校验而非传统 CA 链，因此设备为每个实例生成 EC 自签名证书。
    int ret;
...
{
    // 配置为 DTLS_SIGN_ONCE 时优先复用缓存证书，解析失败再回退到现场生成。
    int ret = 0;
...
{
    // 初始化顺序：引用 SRTP 全局库 -> 创建锁与证书 -> 配置 DTLS 角色 -> 绑定 SRTP profile。
    // 任一步失败都收敛到 dtls_srtp_deinit()，避免密码学对象泄漏。
    dtls_srtp_t *dtls_srtp = (dtls_srtp_t *) media_lib_calloc(1, sizeof(dtls_srtp_t));
    if (dtls_srtp == NULL) {
        return NULL;
    }
    ...
    do {
        ...
        // controlling/controlled 与 DTLS client/server 并非同一概念，角色由协商层显式传入。
...
{
    // 与 init 逆序释放：DTLS 上下文、证书/随机源、角色专属 cookie、双向 SRTP 和锁。
...
{
    // 重连复用证书与随机源，但销毁旧 SRTP 密钥并重置 DTLS 会话；角色变化时重配 client/server。
